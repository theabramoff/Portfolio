...
# 2. in noth VMs - terget and new will be in same domain , then newly deployed VM has to be re-joined to the domain with a different name
# Steps:
# 1. Specify parameters
# 2. Make sure that a target VM is diallocated
# 3. Run the script
# 4. Update domain name if needed
# 5. Start the target VM
# 6. Crosscheck 
#########################################
...
# SQL extention config
# Replace < ... > with SQL license, e.g. PAYG if Developer and AHUB if Standard
$SQLlicense = "< ... >"
# Replace < ... > with SQL SKU, e.g. Developer or Standard
$SQLsku = "< ... >"
...
$SQLVM = Get-AzVM -name $newVM -ResourceGroupName $newresourceGroupName
New-AzSqlVM -Name $SQLVM.name -ResourceGroupName $newresourceGroupName  -Location $location -LicenseType $SQLlicense -Sku $SQLsku -SqlManagementType $SqlManagementType
