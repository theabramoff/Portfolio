#########################################
# Az module required
# this script shows how to get the total size of the blobs in a container
...
# Replace < ... > with a path, e.g. c:\temp\backupJobs
$logpath = "< ... >\log.txt"
...
$listOfBlobs | ForEach-Object {$length = $length + $_.Length / 1073741824 }

$storage = [math]::Round( $length , 2)
...
Write-HostAndLog  "$(Get-Date): Total Space= $storage Gb" $logpath
